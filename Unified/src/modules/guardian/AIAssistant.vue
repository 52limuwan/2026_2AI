...
const handleWebSocketMessage = (message) => {
  ...
  if (message.type === 'llm') {
    ...
    if (ws.getVoiceMode() && message.text && message.text.trim()) {
      ...
      // 立即保存用户消息到数据库
      console.log('📝 保存用户消息 (guardian):', message.text);
      saveMessageToDatabase(userMessage).then(() => {
        userMessage.saved = true
        console.log('✅ 用户消息已保存 (guardian)');
      }).catch(err => {
        console.error('❌ 保存用户消息失败 (guardian):', err);
      })
    }
    ...
    if (message.state === 'start') {
      ...
      // TTS 结束，保存完整的 AI 消息
      console.log('TTS 结束，准备保存AI消息 (guardian)');
      
      if (currentAiMessageIndex.value !== null) {
        const aiMessage = messages.value[currentAiMessageIndex.value]
        if (aiMessage && aiMessage.content && !aiMessage.saved) {
          console.log('📝 保存AI消息 (guardian)，长度:', aiMessage.content.length);
          saveMessageToDatabase(aiMessage).then(() => {
            aiMessage.saved = true
            console.log('✅ AI消息已保存 (guardian)');
          }).catch(err => {
            console.error('❌ 保存AI消息失败 (guardian):', err);
          })
        }
...
const handleClosePhoneCall = () => {
  console.log('handleClosePhoneCall 被调用 (guardian)');
  ...
  if (ws) {
    ...
    // 断开 WebSocket 连接
    console.log('断开 WebSocket 连接 (guardian)');
    ws.disconnect();
  ...
  // 保存所有未保存的消息
  console.log('检查未保存的消息 (guardian)...');
  let unsavedCount = 0;
  messages.value.forEach(msg => {
    if (!msg.saved && msg.content && msg.content.trim()) {
      unsavedCount++;
      console.log(`📝 保存未保存的${msg.role}消息 (guardian):`, msg.content.substring(0, 50) + '...');
      saveMessageToDatabase(msg).then(() => {
        msg.saved = true;
        console.log(`✅ ${msg.role}消息已保存 (guardian)`);
      }).catch(err => {
        console.error(`❌ 保存${msg.role}消息失败 (guardian):`, err);
      });
    }
  });
  
  if (unsavedCount > 0) {
    console.log(`发现 ${unsavedCount} 条未保存的消息 (guardian)，正在保存...`);
  }
