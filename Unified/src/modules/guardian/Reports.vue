<template>
  <div class="card-list">
    <!-- 周报/月报切换器 -->
    <div class="report-header" v-if="selectedClientId">
    ...
    <!-- 本月健康报告 -->
    <div class="report-content" v-if="monthly && activeTab === 'monthly' && selectedClientId">
    ...
    <!-- 本周健康报告 -->
    <div class="report-content" v-if="weekly && activeTab === 'weekly' && selectedClientId">
    ...
    <div class="card empty-state" v-if="!selectedClientId">
      <p>请选择被监护人查看报告</p>
    </div>

    <div class="card empty-state" v-if="selectedClientId && (activeTab === 'weekly' && !weekly) || (activeTab === 'monthly' && !monthly)">
      <p>暂无报告数据</p>
    </div>
  </div>
</template>
...
import { getWeeklyReport, getMonthlyReport, generateWeeklyAiAnalysis, generateMonthlyAiAnalysis } from '../../api/guardian'
...
const loadReports = async () => {
  ...
  try {
    weekly.value = (await getWeeklyReport(selectedClientId.value)).data.data.report
  } catch (err) {
  ...
  try {
    monthly.value = (await getMonthlyReport(selectedClientId.value)).data.data.report
  } catch (err) {
...
const handleClientChanged = (event) => {
  if (event.detail?.client?.id) {
    selectedClientId.value = event.detail.client.id
    loadReports()
  }
}
...
onMounted(async () => {
  ...
  // 检查是否已有选中的被监护人（从 Layout 直接获取）
  if (selectedClient?.value?.id) {
    selectedClientId.value = selectedClient.value.id
    await loadReports()
    await nextTick()
    initCharts()
  }
...
watch(() => selectedClient?.value?.id, (newId) => {
  if (newId && newId !== selectedClientId.value) {
    selectedClientId.value = newId
  }
}, { immediate: true })
...
const formatCalorie = (cal) => {
  if (cal === null || cal === undefined || isNaN(cal)) return '0 kcal'
...
const generateWeeklyAiAnalysisHandler = async () => {
  ...
  try {
    const response = await generateWeeklyAiAnalysis(selectedClientId.value)
...
const generateMonthlyAiAnalysisHandler = async () => {
  ...
  try {
    const response = await generateMonthlyAiAnalysis(selectedClientId.value)
    const analysis = response.data.data.analysis
