<template>
  <div class="card-list">
    ...
    <div class="page-header">
      <h2 class="page-title">{{ clientName }} 的历史报告</h2>
    </div>
...
import { getAiDietReports, getClients } from '../../api/guardian'
...
const loadReports = async () => {
  ...
  try {
    ...
    const response = await getAiDietReports(clientId.value, params)
...
const loadClientInfo = async () => {
  try {
    ...
    const client = clients.find(c => c.id === clientId.value)
    if (client) {
...
const viewReportDetail = (reportId) => {
  router.push(`/guardian/ai-reports/${clientId.value}/${reportId}`)
}
